...
const contactRoute = computed(() => getContactRoute(product.contactProduct, source.value))
</script>
...
<template>
  <div>
    <section class="product-hero" :style="{ '--accent': product.accent }">
      <div class="container">
        ...
        <div class="product-hero-grid">
          <div><p class="eyebrow">{{ product.eyebrow }}</p><h1>{{ product.name }}</h1><p>{{ product.description }}</p><div class="hero-actions"><NuxtLink class="button" :to="contactRoute">获取产品方案</NuxtLink><a class="text-link" href="#capabilities">查看核心能力 ↓</a></div></div>
    ...
    <section id="contact" class="section contact-section"><div class="container contact-card"><div><p class="eyebrow eyebrow-light">产品咨询</p><h2>想进一步了解{{ product.name }}？</h2><p>联系我们，获取产品介绍、适用场景与实施建议。</p></div><NuxtLink class="button button-light" :to="contactRoute">获取产品方案</NuxtLink></div></section>
  </div>
</template>
